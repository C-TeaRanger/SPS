...
class EncoderSimilarity(nn.Module):
    ...
    def glo_emb_p1_I(self,img_emb):
        img_ave = torch.mean(img_emb, 1)
        img_glo = img_ave
        return img_glo

    def glo_emb_p1_T(self,cap_emb,cap_lens):
        cap_emb_all = []
...
class SGRAF(object):
    ...
    def forward_img_emb_p1(self,images):
        if torch.cuda.is_available():
            images = images.cuda()
        img_embs = self.img_enc(images)
        return img_embs
    ...
    def train(
        ...
        if mode == "train_sc":
            ...
            image_g=self.sim_enc.glo_emb_p1_I(img_embs)
            text_g=self.sim_enc.glo_emb_p1_T(cap_embs,cap_lens)

            imageMatrix=SIM_PAIR(image_g,image_g)
            textMatrix=SIM_PAIR(text_g,text_g)

            self.optimizer.zero_grad()

            loss, loss2, col_mean = self.criterion(
                sims,
                labels=labels,
                mode="train",
            ...
            loss3, loss2, col_mean = self.criterion(
                sims,
                labels=labels,
                mode="train_sc",
                soft_label=soft_label,
            ...
            self.optimizer.step()
            return loss.item()+loss2.item(), 0, col_mean,loss.item(),loss2.item(),loss3.item()
    ...
    def process_noisy_p1(self, images_sc,captions_sc,lengths_sc,images_n):


        img_embs, _, _ = self.forward_emb(images_sc, captions_sc, lengths_sc)
        img_g_sc=self.sim_enc.glo_emb_p1_I(img_embs)

        img_n_embs=self.forward_img_emb_p1(images_n)
        img_g_n=self.sim_enc.glo_emb_p1_I(img_n_embs)

        img_cossim=SIM_PAIR(img_g_sc,img_g_n)
        top=img_cossim.topk(k=1,dim=1,largest=True,sorted=True)

        indices = top.indices.squeeze(1)
        batch_text_n = captions_sc[indices]

        batch_lengths_n = [lengths_sc[i.item()] for i in indices]
        img_embs_n, cap_embs_n, cap_lens_n = self.forward_emb(images_n, batch_text_n, batch_lengths_n)
        sims = self.forward_sim(img_embs_n, cap_embs_n, cap_lens_n)
        I = self.criterion(sims, mode="predict")
        p = I.clamp(0, 1)

        corrected_labels = torch.mul(p, top.values.squeeze(1))
        corrected_labels[corrected_labels<0.5]=0
        corrected_labels=corrected_labels.t()

        return corrected_labels, batch_text_n, batch_lengths_n
