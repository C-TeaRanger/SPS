...
from utils import AverageMeter, ProgressMeter,get_logger
...
def evalrank(model_path,logger, data_path=None, vocab_path=None, split="dev", fold5=False):
    """
    ...
    opt = checkpoint["opt"]
    logger.info(f"experiment name: {opt.experiment_name}",)
    logger.info(f"the model's epoch: {checkpoint['epoch']}")
    opt.workers = 0
    print(opt)
    ...
    if not fold5:
        ...
        # logger.info("rsum: %.1f" % (mean_metrics[10] * 6))
        logger.info("rsum: %.1f" % (mean_metrics[0] + mean_metrics[1] + mean_metrics[2]+mean_metrics[5] + mean_metrics[6] + mean_metrics[7]))
...
if __name__ == "__main__":
    ...
    eval_output= os.path.join("eval_output", current_time)
    if not os.path.isdir(eval_output):
        os.makedirs(eval_output)
    logger = get_logger(os.path.join(eval_output, "eval.txt"))

    os.environ["CUDA_VISIBLE_DEVICES"] = "7"

    # model_path = './model_ckpt/cream_models'
    model_path="./Model_to_be_verified/temp"
    # model_path = "./model_ckpt/cream_models/f30k_n4"
    ...
    if os.path.isfile(model_path):
        ...
        checkpoint = torch.load(model_path)
        # ms-coco test
        if checkpoint['opt'].data_name == 'coco_precomp':
            logger.info("----------   ms-coco 1k test   ----------")
            evalrank(
                model_path,
                logger,
                data_path=data_path,
        ...
        # f30k cc152k test
        elif checkpoint['opt'].data_name == 'f30k_precomp' or checkpoint['opt'].data_name == 'cc152k_precomp':
            evalrank(
                model_path,
                logger,
                data_path=data_path,
